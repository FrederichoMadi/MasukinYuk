...
//		perulangan buat nampilin nyawanya
		...
		g.drawImage(hoop, 190, 0, 125, 125, null); // ring basket
	...
	public void playSound(int x) {
		...
		if (x == 1) {
			try {
				...
				clip.open(AudioSystem.getAudioInputStream(new File(System.getProperty("user.dir")+"\\src\\tubes\\endOfGame.wav").getAbsoluteFile()));
				clip.start();
			} catch (Exception ex) {
				ex.printStackTrace();
			}
                    ...
                    try {
                        clip = AudioSystem.getClip();
			clip.open(AudioSystem.getAudioInputStream(new File(System.getProperty("user.dir")+"\\src\\tubes\\Song.wav").getAbsoluteFile()));
                        clip.start();
		...
		// swish sound
			...
			try {
				clip = AudioSystem.getClip();
				clip.open(AudioSystem.getAudioInputStream(new File(System.getProperty("user.dir")+"\\src\\tubes\\swish.wav").getAbsoluteFile()));
				clip.start();
			} catch (Exception ex) {
				ex.printStackTrace();
			}
		...
		for (i = 0; i <= (450 / velX) * 2; i = i + 1) {
			...
			if (lives == 0 && !ENDOFGAME) { // "holds" the loop temporarily so waiting for the player to restart the
				...
				ballX = -10000; // to make it seem like the ball dissapears
                                Db(text.getText(),makes);//username dan score game
...
//      tombol yang dipake untuk bermain
	...
	public void keyPressed(KeyEvent e) {
		...
		if (e.getKeyCode() == KeyEvent.VK_SPACE) { //tombol spasi untuk memainkannya
			MOVING = !MOVING;
		}
	}
